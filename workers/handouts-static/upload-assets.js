...
/**
 * Upload Handouts static assets to R2 bucket
 */
...
async function uploadToR2() {
  try {
    const appsDir = path.join(__dirname, '../../apps/handouts/out');
    ...
    for (const file of files) {
      try {
        ...
        try {
          const cmd = `npx wrangler r2 object put ganger-handouts-assets/${file.key} --file="${tempFile}" --content-type="${contentType}" --remote`;
          execSync(cmd, { 
            stdio: 'pipe',
            env: { 
              ...process.env, 
