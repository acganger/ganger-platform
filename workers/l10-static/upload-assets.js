...
/**
 * Upload L10 static assets to R2 bucket using proven pattern
 */
...
async function uploadToR2() {
  try {
    const appsDir = path.join(__dirname, '../../apps/eos-l10/out');
    ...
    console.log(`🪣 Bucket: ganger-l10-assets`);
    ...
    for (const file of files) {
      try {
        ...
        try {
          const cmd = `npx wrangler r2 object put ganger-l10-assets/${file.key} --file="${tempFile}" --content-type="${contentType}" --remote`;
          execSync(cmd, { 
            stdio: 'pipe',
            env: { 
              ...process.env, 
    ...
    if (failed === 0) {
      ...
      console.log('📋 Next step: Deploy worker with "npx wrangler deploy --env production"');
    } else {
