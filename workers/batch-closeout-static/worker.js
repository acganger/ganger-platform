/**
 * Ganger Platform - Batch Closeout Static Worker (R2 Version)
 * Using proven R2 pattern from EOS L10 and AI Receptionist deployments
 */
    ...
    try {
      ...
      if (pathname === '/health') {
        return new Response(JSON.stringify({
          status: 'healthy',
          app: 'batch-closeout-r2',
          storage: 'r2',
          timestamp: new Date().toISOString(),
          version: '1.0.0',
          environment: env.ENVIRONMENT || 'production',
          binding_available: !!env.STATIC_ASSETS,
          features: ['dashboard_app', 'r2_storage', 'nextjs_routing', 'worker_api_upload', 'batch_processing', 'financial_data']
        }), {
      ...
      if (request.method === 'PUT') {
        ...
        return new Response(`Batch Closeout - Uploaded ${key} successfully!`);
      }
      ...
      if (object === null) {
        ...
        if (indexObject === null) {
          return new Response(`Batch Closeout - File not found: ${key}`, { 
            status: 404,
      ...
      return new Response(`Batch Closeout - Error: ${error.message}`, {
        status: 500,
        headers: { 'Content-Type': 'text/plain' }
      });
    }
