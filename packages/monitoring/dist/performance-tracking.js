...
export function usePerformanceTracking(componentName) {
    ...
    const trackRender = () => {
        ...
        return () => {
            tracker.measure(`${componentName}-render`, `${componentName}-render-start`);
        };
    };
    const trackEffect = (effectName) => {
        const markName = `${componentName}-${effectName}-start`;
        tracker.mark(markName);
        return () => {
            tracker.measure(`${componentName}-${effectName}`, markName);
        };
    };
    const trackApiCall = async (apiName, apiCall) => {
        ...
        try {
            ...
            const measure = tracker.measure(`${componentName}-${apiName}`, markName, undefined, { component: componentName, api: apiName });
            // Log slow API calls
            ...
            tracker.measure(`${componentName}-${apiName}-error`, markName, undefined, { component: componentName, api: apiName, error: String(error) });
            throw error;
        }
    };
    return {
