import { IntegrationHealthStatus, integrationHealthMonitor } from './integration-health';
...
export class HealthAlertingService {
  ...
  private initializeAlertRules(): void {
    this.alertRules = [
      ...
      },
      {
        ...
        condition: (integrations) => integrations.some(i => 
          ['stripe', 'twilio', 'google'].includes(i.service) && i.status === 'degraded'
        ),
  ...
  private shouldTriggerAlert(rule: AlertRule, integrations: IntegrationHealthStatus[]): boolean {
  ...
  private async sendAlert(rule: AlertRule): Promise<void> {
    const eligibleChannels = this.alertChannels.filter(channel => 
      channel.enabled && this.severityMeetsThreshold(rule.severity, channel.severity_threshold)
    );
  ...
  private severityMeetsThreshold(alertSeverity: string, channelThreshold: string): boolean {
    const severityLevels = { low: 1, medium: 2, high: 3, critical: 4 };
    return severityLevels[alertSeverity as keyof typeof severityLevels] >= 
           severityLevels[channelThreshold as keyof typeof severityLevels];
  }

  private async sendSlackAlert(webhookUrl: string, rule: AlertRule): Promise<void> {
    const payload = {
      text: `🚨 ${rule.name}`,
      attachments: [{
  ...
  private async sendEmailAlert(email: string, rule: AlertRule): Promise<void> {
