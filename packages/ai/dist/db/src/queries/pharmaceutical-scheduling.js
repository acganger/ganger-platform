...
exports.PharmaSchedulingQueries = void 0;
// =====================================================
// PHARMACEUTICAL SCHEDULING QUERIES CLASS
// =====================================================
class PharmaSchedulingQueries {
    ...
    async getPharmaRepByEmail(email) {
        const { data, error } = await this.db
            .from('pharma_representatives')
            .select('*')
            .eq('email', email)
    ...
    async getActivePharmaReps(companyFilter, territoryFilter) {
        let query = this.db
            .from('pharma_representatives')
            .select('*')
            .eq('is_active', true)
    ...
    async updatePharmaRep(repId, updates) {
        const updateData = {};
        ...
        if (updates.territory !== undefined)
            updateData.territory = updates.territory;
        ...
        if (updates.preferredLocations)
            updateData.preferred_locations = updates.preferredLocations;
    ...
    async updateRepLastLogin(repId) {
    ...
    // =====================================================
    async createSchedulingActivity(activityData) {
        const { data, error } = await this.db
    ...
    async getSchedulingActivities(locationFilter, activeOnly = true) {
        let query = this.db
            .from('scheduling_activities')
            .select('*')
    ...
    async getSchedulingActivityById(activityId) {
        const { data, error } = await this.db
            .from('scheduling_activities')
            .select('*')
            .eq('id', activityId)
    ...
    async updateSchedulingActivity(activityId, updates) {
        const updateData = {};
        if (updates.activityName)
            updateData.activity_name = updates.activityName;
        if (updates.location)
            updateData.location = updates.location;
        if (updates.locationAddress)
        ...
        if (updates.blockOffMinutes !== undefined)
            updateData.block_off_minutes = updates.blockOffMinutes;
        if (updates.appointmentType)
            updateData.appointment_type = updates.appointmentType;
        if (updates.maxParticipants)
            updateData.max_participants = updates.maxParticipants;
        if (updates.requiresApproval !== undefined)
            updateData.requires_approval = updates.requiresApproval;
        if (updates.isActive !== undefined)
            updateData.is_active = updates.isActive;
        if (updates.availableDays)
            updateData.available_days = updates.availableDays;
        if (updates.availableTimes)
            updateData.available_times = updates.availableTimes;
        if (updates.bookingWindowWeeks)
            updateData.booking_window_weeks = updates.bookingWindowWeeks;
        if (updates.cancellationHours !== undefined)
            updateData.cancellation_hours = updates.cancellationHours;
        if (updates.description !== undefined)
            updateData.description = updates.description;
        if (updates.specialInstructions !== undefined)
            updateData.special_instructions = updates.specialInstructions;
        const { data, error } = await this.db
            .from('scheduling_activities')
            .update(updateData)
            .eq('id', activityId)
    ...
    // =====================================================
    async createPharmaAppointment(appointmentData) {
        const { data, error } = await this.db
    ...
    async getPharmaAppointmentById(appointmentId) {
        const { data, error } = await this.db
            .from('pharma_appointments')
            .select('*')
            .eq('id', appointmentId)
    ...
    async updatePharmaAppointment(appointmentId, updates) {
        const updateData = {};
        if (updates.appointmentDate)
            updateData.appointment_date = updates.appointmentDate;
        ...
        if (updates.participantCount !== undefined)
            updateData.participant_count = updates.participantCount;
        ...
        if (updates.denialReason !== undefined)
            updateData.denial_reason = updates.denialReason;
        ...
        if (updates.confirmationSent !== undefined)
            updateData.confirmation_sent = updates.confirmationSent;
        if (updates.reminderSent !== undefined)
            updateData.reminder_sent = updates.reminderSent;
        if (updates.cancelledAt)
            updateData.cancelled_at = updates.cancelledAt;
        if (updates.cancelledBy)
            updateData.cancelled_by = updates.cancelledBy;
        ...
        if (updates.googleCalendarEventId)
            updateData.google_calendar_event_id = updates.googleCalendarEventId;
        const { data, error } = await this.db
            .from('pharma_appointments')
            .update(updateData)
            .eq('id', appointmentId)
    ...
    async approveAppointment(appointmentId, approvedBy) {
    ...
    async denyAppointment(appointmentId, approvedBy, denialReason) {
        return this.updatePharmaAppointment(appointmentId, {
    ...
    // =====================================================
    async checkAppointmentConflicts(repId, appointmentDate, startTime, endTime, excludeAppointmentId) {
        const { data, error } = await this.db
    ...
    async getAvailableSlots(activityId, startDate, endDate, includeBooked = false) {
        ...
        for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
            ...
            for (const timeRange of availableTimes) {
                ...
                const conflictingAppointments = existingAppointments.filter(apt => apt.appointmentDate === dateStr &&
    ...
    // ANALYTICS QUERIES
    // =====================================================
    async getAnalytics(startDate, endDate, location) {
        let query = this.db
    ...
    async updateAnalytics(date, location) {
        const { error } = await this.db
    ...
    async getLunchConfigByLocation(locationName) {
        const { data, error } = await this.db
    ...
    async getAllLunchConfigs() {
        ...
        if (error)
            throw error;
        return data.map((item) => this.mapLunchConfig(item));
    }
    async updateLunchConfig(locationName, updates, updatedBy) {
        const { data, error } = await this.db
    ...
    async validateLunchTimeSlot(locationName, appointmentDate, startTime) {
        const { data, error } = await this.db
    ...
    async getLunchAppointmentsByLocation(location, startDate, endDate) {
        let query = this.db
            .from('pharma_appointments')
            .select('*')
        ...
        if (error)
            throw error;
        return data.map((item) => this.mapPharmaAppointment(item));
    }
    async checkLunchSlotAvailability(location, appointmentDate, startTime, endTime) {
        ...
        if (existingAppointments.error)
            throw existingAppointments.error;
...
exports.PharmaSchedulingQueries = PharmaSchedulingQueries;
