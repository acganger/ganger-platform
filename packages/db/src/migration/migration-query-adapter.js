...
export class MigrationQueryAdapter {
     ...
     */
    adaptStatusValues(data, tableName) {
     ...
     */
    adaptInputStatusValues(data, tableName) {
        if (!this.config.enableMigrationMode || !data) {
            return data;
        }
        const adapted = { ...data };
     ...
     */
    async select(originalTableName, selectClause = '*', filters, options) {
        ...
        return this.adaptStatusValues(data || [], originalTableName);
    }
    /**
     * Execute an insert query with migration adaptation
     */
    async insert(originalTableName, insertData) {
        const tableName = this.adaptTableName(originalTableName);
        if (this.config.logMigrationQueries) {
     ...
     */
    async update(originalTableName, updateData, filters) {
        const tableName = this.adaptTableName(originalTableName);
        if (this.config.logMigrationQueries) {
     ...
     */
    async delete(originalTableName, filters) {
        const tableName = this.adaptTableName(originalTableName);
        if (this.config.logMigrationQueries) {
