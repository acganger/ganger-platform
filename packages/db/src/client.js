...
class ConnectionMonitor {
    ...
    async alertConnectionPoolHigh() {
        const utilizationPercent = Math.round((this.metrics.totalConnections / dbConfig.maxConnections) * 100);
...
export async function monitoredQuery(queryFn, queryName) {
...
export async function cachedQuery(queryFn, _cacheKey, options = {}) {
...
export async function getPatientCached(patientId) {
...
export async function getLocationCached(locationId) {
    return cachedQuery(async () => {
        const { data, error } = await supabaseAdmin
            .from('locations')
            .select('*')
...
export async function getInventoryByLocationCached(locationId) {
    return cachedQuery(async () => {
        const { data, error } = await supabaseAdmin
...
export async function getUserPermissionsCached(userId) {
    return cachedQuery(async () => {
        const { data, error } = await supabaseAdmin
...
export async function getPatientMedicationsCached(patientId) {
    return cachedQuery(async () => {
        const { data, error } = await supabaseAdmin
...
export async function updatePatientWithCacheInvalidation(patientId, updates) {
...
export async function updateLocationWithCacheInvalidation(locationId, updates) {
    const result = await monitoredQuery(async () => {
        const { data, error } = await supabaseAdmin
            .from('locations')
            .update(updates)
...
export async function updateInventoryWithCacheInvalidation(itemId, updates, _locationId) {
    const result = await monitoredQuery(async () => {
        const { data, error } = await supabaseAdmin
            .from('inventory_items')
            .update(updates)
