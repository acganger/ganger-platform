...
export class StandardizedProductsRepository extends BaseRepository<StandardizedProduct> {
  constructor() {
    super('standardized_products');
  }

  async findAll(onlyActive = true): Promise<StandardizedProduct[]> {
    ...
    return data as StandardizedProduct[];
  }

  async findByCategory(category: ProductCategory, onlyActive = true): Promise<StandardizedProduct[]> {
    let query = this.client
      .from(this.tableName)
      .select('*')
  ...
  async findActive(): Promise<StandardizedProduct[]> {
  ...
  async findCritical(): Promise<StandardizedProduct[]> {
    const { data, error } = await this.client
      .from(this.tableName)
      .select('*')
      .eq('is_critical', true)
  ...
  async findByIds(ids: string[]): Promise<StandardizedProduct[]> {
    const { data, error } = await this.client
      .from(this.tableName)
      .select('*')
      .in('id', ids);
  ...
  async search(query: string): Promise<StandardizedProduct[]> {
    const { data, error } = await this.client
      .from(this.tableName)
      .select('*')
  ...
  async findSubstitutes(productId: string): Promise<StandardizedProduct[]> {
  ...
  async updateUsageStats(productId: string, quantity: number): Promise<void> {
    // Get current product
    const product = await this.findById(productId);
  ...
  async findProductsNeedingReorder(): Promise<StandardizedProduct[]> {
    const { data, error } = await this.client
      .from(this.tableName)
      .select('*')
      .eq('is_active', true)
    ...
    return (data as StandardizedProduct[]).filter(product => {
      if (!product.last_order_date) return true; // Never ordered
      const lastOrderDate = new Date(product.last_order_date);
      return lastOrderDate < thirtyDaysAgo;
    });
  }

  async createProduct(input: CreateStandardizedProductInput): Promise<StandardizedProduct> {
    // Validate input
  ...
  async updateSubstitutes(productId: string, substituteIds: string[]): Promise<StandardizedProduct> {
