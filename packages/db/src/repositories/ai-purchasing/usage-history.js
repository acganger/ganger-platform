 ...
 */
export class UsageHistoryRepository extends BaseRepository {
    constructor() {
     ...
     */
    async getAverageMonthlyUsage(productId, months = 6) {
        const startDate = new Date();
        startDate.setMonth(startDate.getMonth() - months);
        const usageHistory = await this.findByProduct(productId);
        const recentUsage = usageHistory.filter(u => new Date(u.usage_date) >= startDate);
     ...
     */
    async getUsageTrend(productId, months = 3) {
        const usageHistory = await this.findByProduct(productId);
        ...
        usageHistory.forEach(usage => {
            ...
            monthlyUsage.set(monthKey, (monthlyUsage.get(monthKey) || 0) + usage.quantity_used);
        });
        // Get last few months
