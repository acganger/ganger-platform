 ...
 * }
 */
export function createPagesRouterSupabaseClient(req, res) {
    return createServerClient(supabaseUrl, supabaseAnonKey, {
        cookies: {
            ...
            set(name, value, options) {
                ...
                const cookieStr = `${name}=${value}; Domain=${opts.domain}; Path=${opts.path}; ${opts.secure ? 'Secure; ' : ''}SameSite=${opts.sameSite}; ${opts.httpOnly ? 'HttpOnly; ' : ''}Max-Age=${opts.maxAge}`;
            ...
            remove(name, options) {
                const opts = { ...cookieOptions, ...options };
                const cookieStr = `${name}=; Domain=${opts.domain}; Path=${opts.path}; Max-Age=0; ${opts.httpOnly ? 'HttpOnly; ' : ''}`;
