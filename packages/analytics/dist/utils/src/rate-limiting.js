 ...
 */
export function initializeRedisStore(redisClient) {
 ...
 * }
 */
export async function checkRateLimit(req, config) {
 ...
 * );
 */
export function createCombinedRateLimit(config) {
    return {
        ...config,
        keyGenerator: (req) => {
            const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown';
 ...
 * }
 */
export async function isRateLimited(req, config) {
