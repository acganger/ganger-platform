/**
 * Unified Color Token System for Ganger Platform
 *
 * This file defines all color tokens used across the platform to ensure
 * consistency and maintainability. All applications should use these tokens
 * instead of hardcoded color values.
 */
export declare const colors: {
    readonly primary: {
        readonly 50: "#eff6ff";
        readonly 100: "#dbeafe";
        readonly 200: "#bfdbfe";
        readonly 300: "#93c5fd";
        readonly 400: "#60a5fa";
        readonly 500: "#3b82f6";
        readonly 600: "#2563eb";
        readonly 700: "#1d4ed8";
        readonly 800: "#1e40af";
        readonly 900: "#1e3a8a";
        readonly 950: "#172554";
    };
    readonly secondary: {
        readonly 50: "#f0fdf4";
        readonly 100: "#dcfce7";
        readonly 200: "#bbf7d0";
        readonly 300: "#86efac";
        readonly 400: "#4ade80";
        readonly 500: "#22c55e";
        readonly 600: "#16a34a";
        readonly 700: "#15803d";
        readonly 800: "#166534";
        readonly 900: "#14532d";
        readonly 950: "#052e16";
    };
    readonly accent: {
        readonly 50: "#faf5ff";
        readonly 100: "#f3e8ff";
        readonly 200: "#e9d5ff";
        readonly 300: "#d8b4fe";
        readonly 400: "#c084fc";
        readonly 500: "#a855f7";
        readonly 600: "#9333ea";
        readonly 700: "#7c3aed";
        readonly 800: "#6b21a8";
        readonly 900: "#581c87";
        readonly 950: "#3b0764";
    };
    readonly success: {
        readonly 50: "#f0fdf4";
        readonly 100: "#dcfce7";
        readonly 200: "#bbf7d0";
        readonly 300: "#86efac";
        readonly 400: "#4ade80";
        readonly 500: "#22c55e";
        readonly 600: "#16a34a";
        readonly 700: "#15803d";
        readonly 800: "#166534";
        readonly 900: "#14532d";
        readonly 950: "#052e16";
    };
    readonly warning: {
        readonly 50: "#fffbeb";
        readonly 100: "#fef3c7";
        readonly 200: "#fde68a";
        readonly 300: "#fcd34d";
        readonly 400: "#fbbf24";
        readonly 500: "#f59e0b";
        readonly 600: "#d97706";
        readonly 700: "#b45309";
        readonly 800: "#92400e";
        readonly 900: "#78350f";
        readonly 950: "#451a03";
    };
    readonly error: {
        readonly 50: "#fef2f2";
        readonly 100: "#fee2e2";
        readonly 200: "#fecaca";
        readonly 300: "#fca5a5";
        readonly 400: "#f87171";
        readonly 500: "#ef4444";
        readonly 600: "#dc2626";
        readonly 700: "#b91c1c";
        readonly 800: "#991b1b";
        readonly 900: "#7f1d1d";
        readonly 950: "#450a0a";
    };
    readonly info: {
        readonly 50: "#eff6ff";
        readonly 100: "#dbeafe";
        readonly 200: "#bfdbfe";
        readonly 300: "#93c5fd";
        readonly 400: "#60a5fa";
        readonly 500: "#3b82f6";
        readonly 600: "#2563eb";
        readonly 700: "#1d4ed8";
        readonly 800: "#1e40af";
        readonly 900: "#1e3a8a";
        readonly 950: "#172554";
    };
    readonly neutral: {
        readonly 50: "#f8fafc";
        readonly 100: "#f1f5f9";
        readonly 200: "#e2e8f0";
        readonly 300: "#cbd5e1";
        readonly 400: "#94a3b8";
        readonly 500: "#64748b";
        readonly 600: "#475569";
        readonly 700: "#334155";
        readonly 800: "#1e293b";
        readonly 900: "#0f172a";
        readonly 950: "#020617";
    };
    readonly white: "#ffffff";
    readonly black: "#000000";
    readonly transparent: "transparent";
    readonly handouts: {
        readonly education: "#06b6d4";
        readonly treatment: "#f59e0b";
        readonly medication: "#ef4444";
        readonly procedure: "#8b5cf6";
    };
    readonly medical: {
        readonly urgent: "#ef4444";
        readonly critical: "#dc2626";
        readonly processing: "#3b82f6";
        readonly approved: "#22c55e";
        readonly pending: "#f59e0b";
        readonly cancelled: "#6b7280";
    };
    readonly eos: {
        readonly 50: "#f0f9ff";
        readonly 100: "#e0f2fe";
        readonly 200: "#bae6fd";
        readonly 300: "#7dd3fc";
        readonly 400: "#38bdf8";
        readonly 500: "#0ea5e9";
        readonly 600: "#0284c7";
        readonly 700: "#0369a1";
        readonly 800: "#075985";
        readonly 900: "#0c4a6e";
        readonly 950: "#082f49";
    };
    readonly pharma: {
        readonly available: "#22c55e";
        readonly booked: "#3b82f6";
        readonly pending: "#f59e0b";
        readonly cancelled: "#ef4444";
        readonly confirmed: "#059669";
    };
    readonly inventory: {
        readonly inStock: "#22c55e";
        readonly lowStock: "#f59e0b";
        readonly outOfStock: "#ef4444";
        readonly onOrder: "#3b82f6";
        readonly expired: "#dc2626";
    };
};
/**
 * CSS Custom Properties for Dynamic Theming
 * These can be used to enable theme switching if needed in the future
 */
export declare const cssVariables: {
    readonly '--color-primary': "#3b82f6";
    readonly '--color-primary-foreground': "#ffffff";
    readonly '--color-secondary': "#22c55e";
    readonly '--color-secondary-foreground': "#ffffff";
    readonly '--color-accent': "#a855f7";
    readonly '--color-accent-foreground': "#ffffff";
    readonly '--color-success': "#22c55e";
    readonly '--color-warning': "#f59e0b";
    readonly '--color-error': "#ef4444";
    readonly '--color-info': "#3b82f6";
    readonly '--color-background': "#ffffff";
    readonly '--color-foreground': "#0f172a";
    readonly '--color-muted': "#f1f5f9";
    readonly '--color-muted-foreground': "#64748b";
    readonly '--color-border': "#e2e8f0";
    readonly '--color-ring': "#3b82f6";
};
/**
 * Tailwind CSS Color Extension
 * Export colors in format compatible with Tailwind CSS configuration
 */
export declare const tailwindColors: {
    readonly primary: {
        readonly 50: "#eff6ff";
        readonly 100: "#dbeafe";
        readonly 200: "#bfdbfe";
        readonly 300: "#93c5fd";
        readonly 400: "#60a5fa";
        readonly 500: "#3b82f6";
        readonly 600: "#2563eb";
        readonly 700: "#1d4ed8";
        readonly 800: "#1e40af";
        readonly 900: "#1e3a8a";
        readonly 950: "#172554";
    };
    readonly secondary: {
        readonly 50: "#f0fdf4";
        readonly 100: "#dcfce7";
        readonly 200: "#bbf7d0";
        readonly 300: "#86efac";
        readonly 400: "#4ade80";
        readonly 500: "#22c55e";
        readonly 600: "#16a34a";
        readonly 700: "#15803d";
        readonly 800: "#166534";
        readonly 900: "#14532d";
        readonly 950: "#052e16";
    };
    readonly accent: {
        readonly 50: "#faf5ff";
        readonly 100: "#f3e8ff";
        readonly 200: "#e9d5ff";
        readonly 300: "#d8b4fe";
        readonly 400: "#c084fc";
        readonly 500: "#a855f7";
        readonly 600: "#9333ea";
        readonly 700: "#7c3aed";
        readonly 800: "#6b21a8";
        readonly 900: "#581c87";
        readonly 950: "#3b0764";
    };
    readonly success: {
        readonly 50: "#f0fdf4";
        readonly 100: "#dcfce7";
        readonly 200: "#bbf7d0";
        readonly 300: "#86efac";
        readonly 400: "#4ade80";
        readonly 500: "#22c55e";
        readonly 600: "#16a34a";
        readonly 700: "#15803d";
        readonly 800: "#166534";
        readonly 900: "#14532d";
        readonly 950: "#052e16";
    };
    readonly warning: {
        readonly 50: "#fffbeb";
        readonly 100: "#fef3c7";
        readonly 200: "#fde68a";
        readonly 300: "#fcd34d";
        readonly 400: "#fbbf24";
        readonly 500: "#f59e0b";
        readonly 600: "#d97706";
        readonly 700: "#b45309";
        readonly 800: "#92400e";
        readonly 900: "#78350f";
        readonly 950: "#451a03";
    };
    readonly error: {
        readonly 50: "#fef2f2";
        readonly 100: "#fee2e2";
        readonly 200: "#fecaca";
        readonly 300: "#fca5a5";
        readonly 400: "#f87171";
        readonly 500: "#ef4444";
        readonly 600: "#dc2626";
        readonly 700: "#b91c1c";
        readonly 800: "#991b1b";
        readonly 900: "#7f1d1d";
        readonly 950: "#450a0a";
    };
    readonly info: {
        readonly 50: "#eff6ff";
        readonly 100: "#dbeafe";
        readonly 200: "#bfdbfe";
        readonly 300: "#93c5fd";
        readonly 400: "#60a5fa";
        readonly 500: "#3b82f6";
        readonly 600: "#2563eb";
        readonly 700: "#1d4ed8";
        readonly 800: "#1e40af";
        readonly 900: "#1e3a8a";
        readonly 950: "#172554";
    };
    readonly neutral: {
        readonly 50: "#f8fafc";
        readonly 100: "#f1f5f9";
        readonly 200: "#e2e8f0";
        readonly 300: "#cbd5e1";
        readonly 400: "#94a3b8";
        readonly 500: "#64748b";
        readonly 600: "#475569";
        readonly 700: "#334155";
        readonly 800: "#1e293b";
        readonly 900: "#0f172a";
        readonly 950: "#020617";
    };
    readonly eos: {
        readonly 50: "#f0f9ff";
        readonly 100: "#e0f2fe";
        readonly 200: "#bae6fd";
        readonly 300: "#7dd3fc";
        readonly 400: "#38bdf8";
        readonly 500: "#0ea5e9";
        readonly 600: "#0284c7";
        readonly 700: "#0369a1";
        readonly 800: "#075985";
        readonly 900: "#0c4a6e";
        readonly 950: "#082f49";
    };
    readonly handouts: {
        readonly education: "#06b6d4";
        readonly treatment: "#f59e0b";
        readonly medication: "#ef4444";
        readonly procedure: "#8b5cf6";
    };
    readonly medical: {
        readonly urgent: "#ef4444";
        readonly critical: "#dc2626";
        readonly processing: "#3b82f6";
        readonly approved: "#22c55e";
        readonly pending: "#f59e0b";
        readonly cancelled: "#6b7280";
    };
    readonly pharma: {
        readonly available: "#22c55e";
        readonly booked: "#3b82f6";
        readonly pending: "#f59e0b";
        readonly cancelled: "#ef4444";
        readonly confirmed: "#059669";
    };
    readonly inventory: {
        readonly inStock: "#22c55e";
        readonly lowStock: "#f59e0b";
        readonly outOfStock: "#ef4444";
        readonly onOrder: "#3b82f6";
        readonly expired: "#dc2626";
    };
};
/**
 * Color Token Utilities
 */
export declare const colorUtils: {
    /**
     * Get a color value by token path
     * @param path - Color token path (e.g., 'primary.500', 'medical.urgent')
     */
    readonly getColor: (path: string) => string;
    /**
     * Generate CSS custom property name for a color token
     * @param path - Color token path
     */
    readonly getCSSVariable: (path: string) => string;
};
export type ColorToken = keyof typeof colors;
export type ColorScale = typeof colors.primary;
export type SemanticColor = 'success' | 'warning' | 'error' | 'info';
export type ApplicationColor = 'handouts' | 'medical' | 'eos' | 'pharma' | 'inventory';
