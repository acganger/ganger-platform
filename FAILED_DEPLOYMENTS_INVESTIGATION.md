...
All 7 failed deployments have clear root causes:
- **6 apps**: The workspace:* dependencies were converted locally but the error persists in Vercel, suggesting the conversion wasn't deployed
- **1 app (staff)**: Using pnpm with an outdated lockfile

**Important Discovery**: The package.json files show the dependencies were already converted to `file:` paths locally, but Vercel is still seeing `workspace:*`. This means either:
1. The changes weren't committed/pushed
2. Vercel is using cached package.json files
3. The deployment is using an older commit
