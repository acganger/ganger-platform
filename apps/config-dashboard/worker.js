/**
 * Cloudflare Worker for Config Dashboard R2 Deployment
 * Serves static Next.js build from R2 bucket storage
 * Pattern: Proven R2 deployment strategy from EOS L10, AI Receptionist, Batch Closeout, Call Center Ops, Compliance Training
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.CONFIG_DASHBOARD_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.CONFIG_DASHBOARD_BUCKET.get('index.html');
        if (indexObject) {
