/**
 * Cloudflare Worker for config dashboard Next.js Application
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/config/health' || url.pathname === '/health') {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        ...
        message: 'The config dashboard application encountered an error',
        timestamp: new Date().toISOString()
      }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' }
