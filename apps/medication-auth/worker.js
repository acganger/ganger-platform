/**
 * Cloudflare Worker for medication auth Next.js Application
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/meds/health' || url.pathname === '/health') {
      return new Response(JSON.stringify({
        status: 'healthy',
        timestamp: new Date().toISOString(),
        service: 'medication auth',
    ...
    try {
      ...
      return new Response(JSON.stringify({
        ...
        message: 'The medication auth application encountered an error',
        timestamp: new Date().toISOString()
      }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' }
