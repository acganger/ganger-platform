...
export function getGoogleWorkspaceService(): GoogleWorkspaceService {
  if (!googleWorkspaceServiceInstance) {
    const config: GoogleWorkspaceConfig = {
      ...
      targetGroup: process.env.GOOGLE_TARGET_GROUP!,
      targetOU: process.env.GOOGLE_TARGET_OU!
    };

    // Validate required environment variables
    const requiredVars = ['GOOGLE_CLIENT_EMAIL', 'GOOGLE_PRIVATE_KEY', 'GOOGLE_DOMAIN', 'GOOGLE_IMPERSONATE_EMAIL'];
