/**
 * Cloudflare Worker for Socials Reviews R2 Deployment
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.SOCIALS_REVIEWS_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.SOCIALS_REVIEWS_BUCKET.get('index.html');
        if (indexObject) {
