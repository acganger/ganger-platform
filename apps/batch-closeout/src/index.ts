/**
 * Cloudflare Worker for Batch Closeout Platform R2 Deployment
 * Serves static Next.js build from R2 bucket storage with TypeScript type safety
 * Pattern: Modern TypeScript deployment strategy
 */

interface Env {
  BATCH_CLOSEOUT_BUCKET: R2Bucket;
  ENVIRONMENT?: string;
  APP_NAME?: string;
  APP_VERSION?: string;
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      const object = await env.BATCH_CLOSEOUT_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.BATCH_CLOSEOUT_BUCKET.get('index.html');
        if (indexObject) {
