/**
 * Cloudflare Worker for Clinical Staffing Platform R2 Deployment
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.CLINICAL_STAFFING_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.CLINICAL_STAFFING_BUCKET.get('index.html');
        if (indexObject) {
