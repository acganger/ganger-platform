/**
 * Cloudflare Worker for Integration Status App
...
async function handleApiRoute(request, env) {
  ...
  if (path === '/api/integrations') {
    return new Response(JSON.stringify({
      integrations: [
        { name: 'Supabase', status: 'healthy', last_check: new Date().toISOString() },
        { name: 'Google Workspace', status: 'healthy', last_check: new Date().toISOString() },
        { name: 'Stripe', status: 'healthy', last_check: new Date().toISOString() },
        { name: 'Twilio', status: 'warning', last_check: new Date().toISOString() }
      ],
