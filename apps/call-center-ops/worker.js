/**
 * Cloudflare Worker for Call Center Ops Platform R2 Deployment
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.CALL_CENTER_OPS_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.CALL_CENTER_OPS_BUCKET.get('index.html');
        if (indexObject) {
