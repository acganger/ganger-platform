...
# Upload script for R2 bucket with inventory prefix for _next assets
set -e

export CLOUDFLARE_API_TOKEN=TjWbCx-K7trqYmJrU8lYNlJnzD2sIVAVjvvDD8Yf
BUCKET_NAME="ganger-inventory-assets"

echo "📤 Uploading _next files with inventory prefix to R2 bucket: $BUCKET_NAME"
...
# Upload all _next files with inventory prefix
cd out
for file in $(find _next -type f); do
    key="inventory/$file"
