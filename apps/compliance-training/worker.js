/**
 * Cloudflare Worker for Compliance Training R2 Deployment
 * Serves static Next.js build from R2 bucket storage
 * Pattern: Proven R2 deployment strategy from EOS L10, AI Receptionist, Batch Closeout, Call Center Ops
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.COMPLIANCE_TRAINING_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.COMPLIANCE_TRAINING_BUCKET.get('index.html');
        if (indexObject) {
