/**
 * Cloudflare Worker for AI Receptionist Platform R2 Deployment
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const object = await env.AI_RECEPTIONIST_BUCKET.get(key);
      
      if (!object) {
        // Try fallback to index.html for client-side routing
        const indexObject = await env.AI_RECEPTIONIST_BUCKET.get('index.html');
        if (indexObject) {
